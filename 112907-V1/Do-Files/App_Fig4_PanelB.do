...
local poly score15_1 score15_1int  c_* white male

quietly reg recid_`x' agg_dui `poly' if abs(score15_1)<=.05, cluster(score15_1)
gen b_`x'=_b[agg_dui]
gen sd_`x'=_se[agg_dui]
gen up_`x'=b_`x'+1.96*sd_`x'
gen low_`x'=b_`x'-1.96*sd_`x'
...
graph save estimated_effects_repeata.gph, replace 
capture gen elas=b_/recid_



tsline recid, title("Average Recivisim""At Least 1 Prior Test") scheme(s2mono)
graph save recid_rate_repeata.gph, replace
...
graph save elas_repeata.gph, replace


*tsline recid, lpattern(dot)
