...
local poly_dc score8_1 score8_1int  c_* white black hisp male y_* acc age

quietly reg recidivism dui `poly_dc' if abs(score8_1)<`b', cluster(score8_1)
gen bdui_`x'=_b[dui]
gen sd_`x'=_se[dui]
...
local poly_ac score15_1 score15_1int c_* white black hisp male y_*   acc  age

quietly reg recidivism agg_dui `poly_ac' if abs(score15_1)<`b', cluster(score15_1)
...
drop if bandwidth>.068


twoway (tsline bdui_) (tsline up_, lpattern(dash)) (tsline low_, lpattern(dash) title("DUI: Discontinuity at .08") yline(0) xlabel(0(.01).07) scheme(s2mono) legend(label(1 "Est. Effect") label(3 "95% Conf.") order(1 3))) 
graph save dui_band.gph, replace

twoway (tsline bduia_) (tsline upa_, lpattern(dash)) (tsline lowa_, lpattern(dash) title("Agg. DUI: Discontinuity at .15") yline(0) xlabel(0(.01).07) lcolor(black) scheme(s2mono) legend(label(1 "Est. Effect") label(3 "95% Conf.") order(1 3))) 
graph save aggdui_band.gph, replace


graph combine dui_band.gph aggdui_band.gph, title("Bandwidth Choice and Estimated Effects") scheme(s2mono)

graph export bandwidth_choice.eps, replace
